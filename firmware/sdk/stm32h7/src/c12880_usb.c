...
volatile c12880_usb_diagnostics_t g_aginti_c12880_usb_diag = {
    .magic = 0x41475542U,
    .last_bitrate = 1500000U,
};

static int8_t CDC_Init_FS(void) {
  ++g_aginti_c12880_usb_diag.class_init_calls;
...
static int8_t CDC_DeInit_FS(void) {
  ++g_aginti_c12880_usb_diag.class_deinit_calls;
  usb_tx_active = false;
  return (int8_t)USBD_OK;
}
...
static int8_t CDC_Control_FS(uint8_t command, uint8_t *buffer,
                             uint16_t length) {
  ++g_aginti_c12880_usb_diag.control_calls;
  g_aginti_c12880_usb_diag.last_control_command = command;
  g_aginti_c12880_usb_diag.last_control_length = length;
  if (command == CDC_SET_LINE_CODING) {
    if ((buffer == NULL) || (length < 7U)) return (int8_t)USBD_FAIL;
    ++g_aginti_c12880_usb_diag.set_line_coding_calls;
    ...
    g_aginti_c12880_usb_diag.last_bitrate = line_coding.bitrate;
  } else if (command == CDC_GET_LINE_CODING) {
    if ((buffer == NULL) || (length < 7U)) return (int8_t)USBD_FAIL;
    ++g_aginti_c12880_usb_diag.get_line_coding_calls;
  ...
  } else if (command == CDC_SET_CONTROL_LINE_STATE) {
    ++g_aginti_c12880_usb_diag.set_control_line_state_calls;
  }
  return (int8_t)USBD_OK;
}

static int8_t CDC_Receive_FS(uint8_t *buffer, uint32_t *length) {
  ++g_aginti_c12880_usb_diag.receive_packets;
  g_aginti_c12880_usb_diag.receive_bytes += *length;
...
static int8_t CDC_TransmitCplt_FS(uint8_t *buffer, uint32_t *length,
  ...
  ++g_aginti_c12880_usb_diag.transmit_completions;
  usb_tx_active = false;
...
bool c12880_usb_init(void) {
  ++g_aginti_c12880_usb_diag.init_calls;
  g_aginti_c12880_usb_diag.init_stage = 1U;
  usb_rx_head = 0U;
  usb_rx_tail = 0U;
  usb_rx_overrun_count = 0U;
  usb_tx_active = false;
  if (USBD_Init(&hUsbDeviceHS, &AGINTI_Desc, DEVICE_HS) != USBD_OK) return false;
  g_aginti_c12880_usb_diag.init_stage = 2U;
  if (USBD_RegisterClass(&hUsbDeviceHS, &USBD_CDC) != USBD_OK) return false;
  g_aginti_c12880_usb_diag.init_stage = 3U;
  if (USBD_CDC_RegisterInterface(&hUsbDeviceHS, &usb_interface) != USBD_OK)
    return false;
  g_aginti_c12880_usb_diag.init_stage = 4U;
  if (USBD_Start(&hUsbDeviceHS) != USBD_OK) return false;
  g_aginti_c12880_usb_diag.init_stage = 5U;
  return true;
}
...
int c12880_usb_transmit(const uint8_t *data, size_t bytes) {
  ...
  ++g_aginti_c12880_usb_diag.transmit_requests;
  usb_tx_active = true;
